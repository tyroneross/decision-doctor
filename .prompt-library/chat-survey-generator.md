# Chat Survey Generator — Aida

**Version:** v1.0
**Tier:** T2 (Groq Llama-3.x JSON-mode call, structured output)
**Deployment:** backend — `/api/chat/route.ts` calls this when a user accepts the offer-help affordance
...
//   - temperature: 0.2 (small variation OK for survey diversity)
//   - user prompt = the user's decision question + the detector's suggestedPath
```

## Risk notes

- Llama-3-tier may emit stray fence chars; tolerant JSON parse handles it.
- Default values are non-anchoring on purpose — anchoring to a midpoint avoids priming the user before we read their answer.
- Healthcare-PHI guidance is the floor; do not generate fields that solicit PHI even if asked.
