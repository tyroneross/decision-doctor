    ...
    <div className="space-y-5">
                    ...
                    <div
                      className={
                        m.role === "user"
                          ...
                          : "px-4 py-2 rounded-2xl bg-canvas-sunken border border-border max-w-[85%] text-sm whitespace-pre-wrap text-ink"
                      }
                    >
                      {m.content}
                    </div>
      ...
      {promptReducer?.artifact?.promptText && (
        <section className="rounded-2xl border border-border bg-canvas-raised p-5">
          ...
          <pre className="mt-3 rounded-lg bg-canvas-sunken border border-border p-3 text-xs leading-relaxed whitespace-pre-wrap text-ink max-h-64 overflow-auto">
            {promptReducer.artifact.promptText}
          </pre>
          <button
            type="button"
            onClick={() => copy(promptReducer.artifact!.promptText!, "prompt")}
            className="mt-2 inline-flex items-center px-3 py-2 rounded-lg border border-border-strong text-sm text-ink min-h-[44px]"
          >
